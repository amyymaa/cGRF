...
Deriv_Data(:, 1)      = -sin(Spatial*pi/x_end)*(pi/x_end)^2; % analytic derivative of time is second derivative wrt space
...
for i = 2:NumOfTimePoints
    ...
    if i == 2
        % calculate exactly
        Deriv_Data(:,2) = -sin(Spatial*pi/x_end)*(pi/x_end)^2;
        ...
        Deriv_Data(:,i) = Deriv_Data(:, 1) + GP_D2_Mean + ( randn(1, NumOfSpatialPoints)*chol(GP_D2_Var+eye(NumOfSpatialPoints)*1e-8) )';
...
GP_I_Mean = reshape(GP_I_Mean, NumOfSpatialPoints_, NumOfTimePoints) + ...
    repmat(sin(Spatial_*pi/x_end)', 1, NumOfTimePoints);
surf(GP_I_Mean);

StateData( ((j-1)*NumOfSpatialPoints_+1):j*NumOfSpatialPoints_, : ) = GP_I_Mean;

end

%%

% save("heat_Dir_16100.mat", "StateData")
...
xticks([1 21 41 61 81 100])
xticklabels({'0','0.05','0.1','0.15','0.2','0.25'})
yticks([1 4 7 10 13 16])
yticklabels({'0','0.2','0.4','0.6','0.8','1'})


%%

StateData_16100 = load("results/heat_Dir_16100.mat");
rep = 100;
...
nexttile
lb = quantile(StateData_i, quantiles(1), 2);
ub = quantile(StateData_i, quantiles(2), 2);
avg = quantile(StateData_i, 0.5, 2);
func_num_matlab = sol(:, i);
plot_helper(Spatial_, lb, ub, avg, func_num_matlab, xrange1, yrange)
ylabel("u")
title("Dirichlet")
ax = gca;
ax.XTick = [];
...
avg = quantile(StateData_i, 0.5, 2);
func_num_matlab = sol(:, i);
plot_helper(Spatial_, lb, ub, avg, func_num_matlab, xrange2, yrange)
h = ylabel("u");
title("Dirichlet")
ax = gca;
ax.YAxisLocation = 'right';
